...
description: 执行完整 plan 或已批准 spec，并在实施前集中确认 Git、提交、TDD、sub-agent 策略，以及平台支持时的 sub-agent 模型与推理强度。仅当用户明确要求使用 executing-spec-or-plan，或明确选择写有该名称的选项时使用；不要把否定、询问、文档提及或任务匹配视为调用授权。
---
...
“任务拆解”和“TDD／测试方式”维度各自必须且只能标记一个建议项。紧接该维度的选项列表，用一个缩进的 `建议理由：` 段落说明为什么当前任务更适合这个选项；控制在一至两句话，并同时点出主要收益或另一选项更合适的条件。理由必须来自当前 spec／plan、代码库和前置检查，不得只复述选项名称或使用对所有任务都成立的套话。

任务拆解按以下依据建议：

- spec 涉及多个模块、依赖或顺序不明确、改动风险较高，或者用户约束需要在编码前核对时，建议先展示关键步骤并确认；理由说明这能提前发现范围理解、依赖和冲突方面的偏差。
- 任务较小、边界清晰、风险较低且没有值得用户预先决定的实现分岔时，可以建议完全由 Agent 自主；理由说明省去额外交互所带来的效率收益，以及为何不会明显损失用户控制。

TDD／测试方式按以下依据建议：

- 新增行为、修复缺陷或改变可观察行为的重构，并且能建立有价值的自动化测试时，建议使用 `test-driven-development`；理由说明失败测试如何先锁定预期、证明测试有效，并降低回归或返工风险。
- 纯文档、纯配置、一次性探索、生成型产物，或无法先建立有效自动化测试时，可以建议不调用该技能、由 Agent 按风险安排测试；理由说明为什么严格 RED–GREEN–REFACTOR 在这里收益有限，同时明确仍会采用何种层级的验证。

若 plan 与 TDD 顺序冲突，TDD 维度的建议理由还要说明调整顺序或保持 plan 的具体代价，不得把“建议使用 TDD”表述为无条件结论。
...
2. 任务拆解
   ...
   - `2A` 先展示关键步骤并再次确认（建议）
   - `2B` 完全由 Agent 自主拆解和执行

   建议理由：该 spec 涉及多个相互依赖的模块，先确认关键步骤可以在编码前发现范围或顺序偏差；如果只是边界清晰的小改动，自主执行会更省交互。

4. 测试方式

   - `4A` 使用 `test-driven-development` 执行 RED–GREEN–REFACTOR（建议）
   - `4B` 不调用该技能，由 Agent 根据风险安排测试

   建议理由：本次会改变可观察行为，并且可以先用自动化测试复现预期失败；TDD 能证明测试会捕获问题并降低后续回归风险，而纯文档或无法建立有效失败测试的任务更适合按风险验证。
...
2. **明确强度**——依次列出平台当前明确支持的 `low`、`medium`、`high`、`xhigh`、`max`、`ultra`；平台未明确支持的值不得显示。
