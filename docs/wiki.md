...
- 先检测平台能否为 sub-agent 单独指定模型，仅在支持时让用户选择继承当前会话模型或某个可用模型；
...
模型能力必须从当前平台的 sub-agent 调用接口、参数 schema 或能力元数据判断。能创建 sub-agent 不代表能覆盖其模型；Agent 不会使用硬编码列表或根据其他平台经验猜测。
...
| sub-agent 模型 | 与当前会话相同，或平台明确列出的某个可用模型 | 只在平台支持为 sub-agent 单独指定模型时出现；作为本次执行的统一默认值 |
...
完整 plan 路径集中收集四个主要决定，以及最多两个条件决定：
...
| sub-agent 模型 | 与当前会话相同／平台当前明确列出的各个可用模型；仅在支持单独指定时出现 |
...
flowchart TD
    ...
    G --> C["集中确认：位置、提交、sub-agent、模型（若支持）、TDD"]
    C --> K{"存在 TDD 顺序冲突？"}
...
- 若平台支持单独指定模型，仍使用第一轮已经确认的 sub-agent 模型；
...
这意味着 sub-agent 策略确认是条件性的：只有用户要求预先审阅任务拆解时才出现。用户把拆解完全交给 Agent 时，调度也一并交给 Agent。模型选择是另一个条件维度：平台支持为 sub-agent 单独指定模型时才出现，无论最终是否实际派遣；若选择禁止使用或最终没有派遣，该模型选择不产生调用。

### sub-agent 模型选择

模型选项按固定语义、动态内容生成：

1. 第一项始终是“与当前会话相同”，通过继承或默认行为实现，不传模型覆盖值；
2. 后续选项逐一列出平台当前明确暴露的可用模型标识，每个模型使用独立选项；
3. 平台未提供的模型不得凭经验补充；
4. 用户的选择作为本次执行中所有 sub-agent 的统一默认值，不改变当前 Agent；
...
- 平台支持时，只能列出当前能力元数据能够验证的模型，派遣失败时不得静默替换。
