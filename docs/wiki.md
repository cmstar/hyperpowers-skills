...
确认面板不能只在任务拆解和 TDD 选项后标注“建议”。这两个维度各自还会用一至两句话解释当前建议的依据、主要收益，以及另一选项更适合的条件。拆解建议主要依据模块与依赖数量、范围清晰度、风险和实现分岔；TDD 建议主要依据任务是否改变可观察行为、能否先建立有价值的失败测试，以及严格 RED–GREEN–REFACTOR 的实际收益。纯文档、纯配置、一次性探索或无法建立有效自动化测试的任务不会机械推荐 TDD，但仍会说明替代验证方式。
