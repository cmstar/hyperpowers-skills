...
本文档说明 Hyperpowers 与原版 [Superpowers](https://github.com/obra/superpowers) 的主要差异、五个精简重构核心技能的调整原因、独立扩展技能，以及项目的完整工作流程。

## 项目定位

Superpowers 提供了一套覆盖设计、规划、实现、测试、调试、review、分支收尾和 sub-agent 调度的完整技能体系。多个技能会自动衔接，使 Agent 能够在较少人工干预下持续推进。

Hyperpowers 以其中五个核心能力的精简重构作为基础：
...
此外，Hyperpowers 可以加入原版没有、但符合项目定位的独立扩展。例如 `git-auto-commit` 用于根据当前提交范围、项目规则和近期历史生成提交信息并创建 Git commit。
...
|---|---|---|---|
| 技能范围 | 覆盖完整开发生命周期 | 以五个精简重构核心技能为基础，并允许独立扩展 | 减少固定流程，同时按需补充能力 |
...
- 分别检测平台能否为 sub-agent 单独指定模型与推理强度，仅在支持相应能力时收集对应选择；
...
`git-auto-commit` 不来自原版，是 Hyperpowers 当前提供的独立扩展。它负责：
...
- 平台是否支持 sub-agent，以及是否支持为单个 sub-agent 指定模型；
- 支持单独指定模型时，平台当前明确暴露的可用模型标识；
- 平台是否支持为单个 sub-agent 指定推理强度，以及可用强度标识和模型兼容信息。

模型与推理强度能力必须分别从当前平台的 sub-agent 调用接口、参数 schema 或能力元数据判断。能创建 sub-agent 不代表能覆盖模型或推理强度，能覆盖模型也不代表能覆盖推理强度；Agent 不会根据其他平台经验猜测未暴露的能力或兼容关系。
...
| sub-agent 模型 | 默认，或平台明确列出的某个可用模型 | 只在平台支持为 sub-agent 单独指定模型时出现；作为本次执行的统一策略 |
| sub-agent 推理强度 | 默认，或平台明确支持的 `low` 至 `ultra` | 只在平台支持为 sub-agent 单独指定推理强度时出现；作为本次执行的统一策略 |
...
完整 plan 路径集中收集四个主要决定，以及最多三个条件决定：
...
| sub-agent 模型 | 默认／平台当前明确列出的各个可用模型；仅在支持单独指定时出现 |
| sub-agent 推理强度 | 默认／`low`／`medium`／`high`／`xhigh`／`max`／`ultra`；仅列出平台明确支持的值，并且只在支持单独指定时出现 |
...
flowchart TD
    ...
    G --> C["集中确认：位置、提交、sub-agent、模型与推理强度（若支持）、TDD"]
    C --> K{"存在 TDD 顺序冲突？"}
...
- 若平台支持相应覆盖能力，仍使用第一轮已经确认的 sub-agent 模型与推理强度；
...
这意味着 sub-agent 策略确认是条件性的：只有用户要求预先审阅任务拆解时才出现。用户把拆解完全交给 Agent 时，调度也一并交给 Agent。模型与推理强度是两个相互独立的条件维度：平台支持相应覆盖能力时才出现，无论最终是否实际派遣；若选择禁止使用或最终没有派遣，已经收集的模型与推理强度选择都不产生调用。

### sub-agent 模型选择

模型选项按固定语义、动态内容生成：

1. 第一项始终是“默认”；确认面板不强加固定模型覆盖值，派遣时先应用 `AGENTS.md`、平台配置等现有规则，没有额外指定时才继承当前会话模型；
2. 后续选项逐一列出平台当前明确暴露的可用模型标识，每个模型使用独立选项；
3. 平台未提供的模型不得凭经验补充；
4. 用户的选择作为本次执行中所有 sub-agent 的统一默认策略，不改变当前 Agent；明确模型产生固定覆盖值，“默认”则让各 sub-agent 按适用配置解析；
5. 派遣时所选模型已不可用，则暂停并重新询问，不静默回退。

如果平台不支持单独指定模型，这个维度不会出现在确认面板中，也不会要求用户提供无效数据。

确认面板中的选项显示为“默认（遵守现有配置；未指定时与当前会话相同）”。“默认”不是强制使用当前会话模型；若现有规则指定了模型，派遣时仍按平台接口要求传递或应用该值。

### sub-agent 推理强度选择

推理强度是独立于模型的确认维度。只有平台明确支持为单个 sub-agent 覆盖推理强度时才显示，并按以下顺序生成选项：

1. 第一项始终是“默认”；确认面板不强加固定推理强度覆盖值，派遣时先应用 `AGENTS.md`、平台配置等现有规则，没有额外指定时才继承当前会话推理强度；
2. 后续依次列出平台当前明确支持的 `low`、`medium`、`high`、`xhigh`、`max`、`ultra`；
3. 如果平台只明确支持其中一部分，只显示可验证的值，不猜测或补齐；
4. 用户的选择作为本次执行中所有 sub-agent 的统一默认策略，不改变当前 Agent；明确强度产生固定覆盖值，“默认”则让各 sub-agent 按适用配置解析；
5. 选择禁止使用或最终没有派遣 sub-agent 时，该选择不生效。

例如该维度在确认面板中排第 5 时，使用 `5A` 表示“默认（遵守现有配置；未指定时与当前会话相同）”，再以 `5B` 至 `5G` 依次表示 `low` 至 `ultra`；实际维度编号仍随当前未决项动态生成。“默认”不是强制使用当前会话推理强度；若现有规则指定了强度，派遣时仍按平台接口要求传递或应用该值。

每次派遣前重新核对所选模型与推理强度的兼容性。若所选模型不支持该强度、强度已不可用，或调用返回不兼容错误，Agent 会暂停并重新确认受影响的维度，不会静默降低强度、移除覆盖值或替换模型。重新确认时模型与推理强度仍是两个独立维度；若两项都需重选，Agent 会分别列出选项并标注兼容范围，不会把模型与强度打包成组合选项。

```mermaid
flowchart TD
    S["读取 approved spec"] --> G["检查 Git、branch 与工作区"]
    G --> C["第一轮：位置、拆解、提交、TDD、模型与推理强度（若支持）"]
    C --> D{"是否预先确认拆解？"}
...
- 平台不支持为 sub-agent 单独指定模型时，不显示或收集模型维度；
- 平台不支持为 sub-agent 单独指定推理强度时，不显示或收集推理强度维度；
- 平台支持时，只能列出当前能力元数据能够验证的模型与推理强度，派遣失败或组合不兼容时不得静默替换或降级。
...
3. 报告：
   ...
   - 未提交文件；
   - 当前 branch；
   - sub-agent 模型与推理强度选择及实际使用情况（若收集过）；
   - worktree 路径（如果创建过）。
